...
fn mark_antinodes(a: (usize, usize), b: (usize, usize), width: usize, height: usize, nodemap: &mut Vec<(usize, usize)>) -> u32 {
        ...
        for x in 0..width {
            if map[y][x] != '.' {
                ...
                let v = find_matches(character, start_pos, &map, width, height); {
                    for pair in v {
                        unique_nodes += mark_antinodes(start_pos, pair, width, height, &mut nodemap);
                    }
                }
            }
        }
    }
    println!("Part 1 unique nodes: {}", unique_nodes);
}